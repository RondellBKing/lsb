...
class Oklahoma(Scraper):
    def __init__(self, start_date=None, delta=5):
        super().__init__(start_date, delta)
        self.county_name = "oklahoma"

    def scrape(self):
        browser = drivers.create_driver('https://www.okcc.online/') # By Pass document type search
        
        # Click Fed Tax Lien
        WebDriverWait(browser, 10).until(EC.element_to_be_clickable((By.XPATH, '//*[@id="ucc-menu-button"]'))).click()
        # Click Notice of Fed Lien
        WebDriverWait(browser, 10).until(EC.element_to_be_clickable((By.XPATH, '//*[@id="uccDocTypeTxt"]'))).send_keys('TL1 - FEDERAL TAX LIEN')
                ...
                for row in list_iterator:
                    ...
                    if row.get_text() != '\nInternal Revenue Service\n':
                        if len(row) == 8 : # Columns with rows properly formatted have len == 8
                            ...
                            lead_list.append([date, combined_lead_names, 'LSB', 'OK', cty])
